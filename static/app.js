...
const REAL_FINAL_SCORES = {
    1: { awayScore: 2, homeScore: 5 },
    2: { awayScore: 4, homeScore: 8 },
    3: { awayScore: 3, homeScore: 6 },
    4: { awayScore: 5, homeScore: 4 },
    5: { awayScore: 6, homeScore: 3 },
    6: { awayScore: 7, homeScore: 2 },
    7: { awayScore: 4, homeScore: 3 },
    8: { awayScore: 2, homeScore: 5 },
    9: { awayScore: 6, homeScore: 4 },
    10: { awayScore: 3, homeScore: 7 },
    11: { awayScore: 9, homeScore: 2 },
    12: { awayScore: 4, homeScore: 5 },
    13: { awayScore: 8, homeScore: 3 },
    14: { awayScore: 3, homeScore: 5 }
};
    ...
    container.innerHTML = `
        ...
        <div class="parlays-grid">
            ...
            <div class="parlay-card">
                ...
                <button class="btn-parlay-action" onclick="registerParlayToAudit('Trinca Hándicap -1.5 (Phillies + Astros + Padres)', '+540')">Registrar Parlay Hándicap en Auditoría</button>
            </div>
        </div>
    `;
}
...
function registerTrendPick(away, home) {
    ...
    const newRecord = {
        ...
        date: CURRENT_DATE,
        match: `${away} vs ${home}`,
        selection: "Análisis de Tendencia",
        prob: "68.2%",
        result: "Pendiente",
        status: "PENDIENTE"
    };
    ...
    renderAuditHistory();
    alert("¡Tendencia registrada exitosamente en la auditoría!");
}
...
function registerParlayToAudit(parlayName, odds) {
    ...
    saveAuditRecords(records);
    renderAuditHistory();
    alert("¡Parlay registrado en la auditoría correctamente!");
}
...
function renderAuditHistory() {
    ...
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:0.8rem;">
            ${records.map(r => {
                let statusColor = r.status === 'ACERTADO' ? '#34d399' : (r.status === 'FALLADO' ? '#ef4444' : '#38bdf8');
                return `
                    <div style="background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.08); padding:1rem; border-radius:10px; display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            <div style="font-size:0.75rem; color:#9ca3af;">📅 ${r.date} &bull; ID: ${r.id}</div>
                            <div style="font-weight:700; color:#fff; font-size:0.95rem; margin:0.2rem 0;">${r.match}</div>
                            <div style="font-size:0.8rem; color:#38bdf8;">Selección: ${r.selection} (${r.prob})</div>
                        </div>
                        <div style="text-align:right;">
                            <div style="font-weight:800; font-size:0.85rem; color:${statusColor};">${r.status}</div>
                            <div style="font-size:0.75rem; color:#9ca3af; margin-top:0.2rem;">${r.result}</div>
                        </div>
                    </div>
                `;
            }).join('')}
